...
module CF::Harness
  class App
    ...
    def inspect
      "#<CF::Harness::App '#@name' '#@manifest'>"
    end

    def push(services = nil, app_manifest = {}, need_check = true)
      set_manifest(app_manifest) unless @manifest
    ...
    def bind(service, restart_app = true)
      ...
      begin
        ...
        @app.bind(service.instance) unless @app.binds?(service.instance)
      rescue Exception => e
    ...
    def load_manifest
      raise "Nil manifest" unless @manifest
      @manifest
    end

    def set_manifest(app_manifest)
        %w[runtime framework memory path].each do |item|
          raise "manifest should contain #{item}" unless app_manifest[item]
        end
        @manifest = app_manifest
        @manifest['instances'] = manifest['instances'] ? manifest['instances'].to_i : 1
        @manifest['memory'] = manifest['memory'] ? manifest['memory'].to_i : 128
    end
    ...
    def check_application(timeout_secs=10)
      seconds = 0
      until @app.healthy?
        sleep 1
        seconds += 1
